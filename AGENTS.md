...
- **Скачивание работает только в локальной сессии на машине пользователя.** В облачной сессии нет ни `yt-dlp`, ни файлов репозитория — обходить это не нужно, надо сказать пользователю, что требуется локальный запуск.
- **Не пытаться получить субтитры через браузер, WebFetch или веб-поиск.** Даже локально это упирается в прокси и создаёт ложный вывод «YouTube недоступен». У `yt-dlp` прямой доступ есть — проверено 2026-08-29 из PowerShell и из Bash, системный прокси выключен.
